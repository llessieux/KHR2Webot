...
char *joint_number_to_name[JOINT_COUNT] =
{
    "Head",
...
void rcb3_simulator::init()
{
    int i;
    m_current_motion = NO_MOTION_INDEX;
    m_robot_name = robot_get_name();

    for (i = 0; i < JOINT_COUNT; i++) {
        m_joint[i] = robot_get_device(joint_number_to_name[i]);
        servo_enable_position(m_joint[i], TIME_STEP);
    }
    ...
    for (i = 0; i < JOINT_COUNT; i++){
        if (m_joint[i] == 0) {
            IfDebugLevel(0) robot_console_printf("Joint %d is missing", i);
        }
        else
        {
            servo_set_acceleration(m_joint[i],-1);
            servo_set_velocity(m_joint[i],servo_speed);
        }
    }


    m_accelerometerTag = robot_get_device("RCB3_ACCELEROMETER");
    receiver_enable(m_accelerometerTag, TIME_STEP);

    m_gyroPitchTag = robot_get_device("RCB3_GYRO_1");
    receiver_enable(m_gyroPitchTag, TIME_STEP);

    m_gyroRollTag = robot_get_device("RCB3_GYRO_2");
    receiver_enable(m_gyroRollTag, TIME_STEP);

    robot_keyboard_enable(TIME_STEP);

    IfDebugLevel(0) robot_console_printf("Accelerometer TAG %d \n",m_accelerometerTag);
    ...
    static char *commands[] ={
        ...
        "V= Walk backword\n",
        "B = Turn Right ?\n",
        "Alt R = Stand Up (from face down or up)\n",
        "Ctrl V = Stand up (face down)\n",
        "Shift 8 = Forward Wheel\n",
        "Shift 2 = Backward Wheel\n",
        "Shift 4 = Left Wheel\n",
        "Shift 6 = Right Wheel\n",
        "5 = Home made scenario showing all the motions.\n",
        "=======================================\n",
        "Commands:\n",
        "=======================================\n",
        " [ to increase the delay during steps (allow slow motion view)\n",
        " ] to decrease the delay during steps \n",        
        " \"SPACE\" set the delay to zero\n",
        " Q to Stop a scenario\n",
        "Escape to Exit\n",
        NULL
    };
    ...
    {
        IfDebugLevel(0) robot_console_printf(commands[c]);
        printf(commands[c]);
        c++;
    }
}
...
void rcb3_simulator::play_scenario(unsigned int store_index)
{
    ...
    {
        IfDebugLevel(1) robot_console_printf("playing scenario %d\n",store_index);
...
void rcb3_simulator::play_motion(unsigned int store_index)
{
    ...
    {
        IfDebugLevel(1) robot_console_printf("playing motion %d\n",store_index);
...
void rcb3_simulator::run()
{
    RCBMotion *m = NULL;
    {
        ...
        if ( m )
        {
            ...
            IfDebugLevel(1) robot_console_printf("Playing Motion %s \n",m->m_name);
            m->m_current_pos = m->m_start;
            int pos = m->m_start;

            double start_time = robot_get_time();
            m_motion_mixing_count = 0;
            float estimated_motion_time = 0;
            while(pos != -1)
            {
                activate_widget(m_current_scenario_position,pos);
                IfDebugLevel(2) robot_console_printf("Position %d\n",pos);
                RCBMotionItem mt = m->m_items[pos];
                IfDebugLevel(3) robot_console_printf("Motion Item %d\n",pos);
                ...
                for(j=0;j<CHANNEL_COUNT_RCB3;j++)
                {
                    if ( channel_indices[j] != -1 )
                    {
                        if ( *it >= 32768 )
                        {
                            ...
                            IfDebugLevel(4) robot_console_printf("Joint %d : %d\n",j,*it- 16384);
                    ...
                    if ( command >= 32768 )
                    {
                        ...
                        if ( j == 0 )
                        {
                            if (( command >= 33287 ) && ( command <= 33542))
                            {
                                ...
                                IfDebugLevel(4) robot_console_printf("Setting Loop Counter to %d\n",m_loop_counter);
                            }
                            ...
                            if (( command >= 33543 ) && ( command <= 33798))
                            {
                                ...
                                IfDebugLevel(4) robot_console_printf("Updating Loop Counter to %d\n",m_loop_counter);
                                if ( m_loop_counter != 0 )
                                    default_jump = false;
                            }
                            ...
                            if (( command >= 39942 ) && ( command <= 57348))
                            {
                                ...
                                IfDebugLevel(4) robot_console_printf("Setting Compare Register to %d\n",m_compare_register);
                            }
                            ...
                            if (( command >= 35846 ) && ( command <= 36101))
                            {
                                ...
                                IfDebugLevel(4) robot_console_printf("Checking AD1=%d against register=%d\n",m_ad[0],m_compare_register);
                            ...
                            if (( command >= 36102 ) && ( command <= 36357))
                            {
                                ...
                                IfDebugLevel(4) robot_console_printf("Checking AD2=%d against register=%d\n",m_ad[1],m_compare_register);
                            ...
                            if (( command >= 36358) && ( command <= 36613))
                            {
                                ...
                                IfDebugLevel(4) robot_console_printf("Checking AD3=%d against register=%d\n",m_ad[2],m_compare_register);
                            ...
                            if (( command >= 39430) && ( command <= 39685))
                            {
                                ...
                                IfDebugLevel(4) robot_console_printf("Checking if button(simulated remote control %d) == register=%d\n",m_button,m_compare_register);
                            ...
                            if (( command >= 39686) && ( command <= 39941))
                            {
                                ...
                                //check button vs register
                                //int rcb3_register = command - 35846;
                                IfDebugLevel(4) robot_console_printf("Checking button(simulated remote control %d) bitwise AND register=%d\n",m_button,m_compare_register);
                                if ( m_button & m_compare_register )
                                    default_jump = false;
                            }
                            ...
                            {
                                recognized = true;
                                IfDebugLevel(4) robot_console_printf("AD1 Mixing\n");
                            ...
                            {
                                recognized = true;
                                IfDebugLevel(4) robot_console_printf("AD2 Mixing\n");
                            ...
                            {
                                recognized = true;
                                IfDebugLevel(4) robot_console_printf("AD3 Mixing\n");
                            ...
                            {
                                recognized = true;
                                IfDebugLevel(4) robot_console_printf("Realtime AD1 Mixing\n");
                            ...
                            {
                                recognized = true;
                                IfDebugLevel(4) robot_console_printf("Realtime AD2 Mixing\n");
                            ...
                            {
                                recognized = true;
                                IfDebugLevel(4) robot_console_printf("Realtime AD3 Mixing\n");
                            ...
                            if (( command >= 37382 ) && ( command <= 37637))
                            {
                                ...
                                IfDebugLevel(4) robot_console_printf("Checking PAD1=%d against register=%d\n",m_ad[0]-m_ad_origin[0],m_compare_register);
                            ...
                            if (( command >= 37638 ) && ( command <= 37893))
                            {
                                ...
                                IfDebugLevel(4) robot_console_printf("Checking PAD2=%d against register=%d\n",m_ad[1]-m_ad_origin[1],m_compare_register);
                            ...
                            if (( command >= 37894) && ( command <= 38149))
                            {
                                ...
                                IfDebugLevel(4) robot_console_printf("Checking PAD3=%d against register=%d\n",m_ad[2]-m_ad_origin[2],m_compare_register);
                        ...
                        if (( j == 0 ) && !recognized )
                        {
                            IfDebugLevel(3) robot_console_printf("Unrecognized command %d\n",command);
                        }
                    }
                ...
                for(j=0;j<CHANNEL_COUNT_RCB3;j++)
                {
                    if ( m_desired_joint_position[j] != m_joint_position[j])
                    {
                        ...
                        if ( ch != -1 )
                        {
							start_position[ch] = servo_get_position(m_joint[ch]);
                            deltas[ch] = delta;
                            IfDebugLevel(5) robot_console_printf("Moving %s to %f\n",joint_number_to_name[ch],m_desired_joint_position[j]);
                            float new_angle = m_desired_joint_position[j] * (float)M_PI / 180.0f;
                            servo_set_position(m_joint[ch],new_angle );
                ...
                for(int s=0;s<JOINT_COUNT;s++)
                {
                    if ( deltas[s] != 0.0f )
                    {
                        ...
                        if ( reduction > 1.0f )
                            reduction = 1.0f;
                        servo_set_velocity(m_joint[s],servo_speed*reduction);                        
                    }
                }

                IfDebugLevel(5) robot_console_printf("Remaining time %f\n",remaining_exec_time);
                ...
                while( !done )
                {
                    ...
                    for(j=0;j<CHANNEL_COUNT_RCB3;j++)
                    {
                        ...
                        if ( ch != -1 )
                        {
                            ...
                            {
                                float new_angle = m_desired_joint_position[j] * (float)M_PI / 180.0f;
                                float reached_angle = servo_get_position(m_joint[ch]);
                    ...
                    get_ad_values();
                    m_button = convert_to_remote_code(robot_keyboard_get_key());
                ...
                IfDebugLevel(2) robot_console_printf("\n");
                pos = m->GetNextItem(default_jump);
                if ( pos == NO_MOTION_INDEX )
                    break;
            }

            double end_time = robot_get_time();
            IfDebugLevel(2) robot_console_printf("Motion executed in %f s (estimated %f)\n",(float)(end_time-start_time),estimated_motion_time/1000.0f);
        }
        ...
        int pressed_key = convert_to_remote_code(robot_keyboard_get_key());
...
int rcb3_simulator::convert_to_remote_code(int key)
{
    ...
    int ctrl = key & ROBOT_KEYBOARD_CONTROL;
    int alt = key & ROBOT_KEYBOARD_ALT;
    int shift = key & ROBOT_KEYBOARD_SHIFT;
    ...
    int normal_key = key & ROBOT_KEYBOARD_KEY;
    ...
    case 'Q':
        IfDebugLevel(1) robot_console_printf("Scenario stopped: %d\n",m_simulation_delay);
        m_current_scenario = NO_MOTION_INDEX;
        break;   
    ...
    case '[':        
        if ( m_simulation_delay > 0 )
        {
            ...
            IfDebugLevel(1) robot_console_printf("New simulation step delay %d ms\n",m_simulation_delay);
        }
        break;
    case ']':
        m_simulation_delay += 10;
    ...
    case ' ':
        m_simulation_delay = 0;
        IfDebugLevel(1) robot_console_printf("Simulation step delay set to 0 ms\n");
        break;

    }

    
    IfDebugLevel(3) robot_console_printf("Key %d (%c) received,Shift : %d Alt %d Control %d -->  Code %d returned \n",
...
bool rcb3_simulator::load_scenario(const char *filename,unsigned int store_index)
{
    ...
    printf("Loading %s in scenario slot %d\n",filename,store_index);
...
bool rcb3_simulator::load_motion(const char *filename,unsigned int store_index)
{
    ...
    printf("Loading %s in motion slot %d\n",filename,store_index);
...
{
    int rec_size = receiver_get_buffer_size(m_accelerometerTag);
    if ( rec_size >= (int)sizeof(dReal) )
    {
        dReal *accel_value = (dReal *)receiver_get_buffer(m_accelerometerTag);
        //Now convert that value into the same output from the RCB3 Analog 2 Digital converter
        dReal accel = *accel_value;
        m_ad[2] = (int)(accel * 55) + 380;
        IfDebugLevel(5) robot_console_printf("new ad3 value : %d (%f)\n",m_ad[2],accel);
    }

    rec_size = receiver_get_buffer_size(m_gyroRollTag);
    if ( rec_size >= (int)sizeof(dReal) )
    {
        dReal *roll_ptr = (dReal *)receiver_get_buffer(m_gyroRollTag);
        ...
        IfDebugLevel(5) robot_console_printf("new ad2 value : %d (%f)\n",m_ad[1],roll);
    }

    rec_size = receiver_get_buffer_size(m_gyroPitchTag);
    if ( rec_size >= (int)sizeof(dReal) )
    {
        dReal *pitch_ptr = (dReal *)receiver_get_buffer(m_gyroPitchTag);
        //Now convert that value into the same output from the RCB3 Analog 1 Digital converter
        dReal pitch = *pitch_ptr;
        m_ad[0] = (int)(pitch * 1024) / 5; //10 bit value for a voltage between 0 and 5v.
        IfDebugLevel(5) robot_console_printf("new ad1 value : %d (%f)\n",m_ad[0],pitch);
    }
}
...
{                                
    ...
    if ( ad_mixing )
    {
        IfDebugLevel(4) robot_console_printf("Activating mixing on channel : %d Input %d\n",channel ,ad+1);
    }
        ...
        for(int ad=0;ad<3;ad++)
        {
            ...
            if ( mixing )
            {
                ...
                if ( delta != 0 )
                {
                    ...
                    IfDebugLevel(4) robot_console_printf("Mixing: channel %d Adding %d\n",j,mod);
                }
            }
        }
    }
...
RCBMotion::GetNextItem(bool default_jump)
{
    ...
    for(int l=0;l<m_link_count;l++)
    {
        ...
        {                        
            ...
            {
                IfDebugLevel(3) robot_console_printf("Link %d\n",link.m_index);
